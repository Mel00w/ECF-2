...
session_start();
$id_user = $_SESSION['id_user'] ?? null; // Assure-toi que l'ID utilisateur est bien récupéré de la session

// Requête préparée pour récupérer les infos de l'utilisateur
...
$id_user = $_SESSION['id_user'] ?? null; // Récupère l'ID de l'utilisateur connecté depuis la session
...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $req = $bdd->prepare('
        SELECT post.`id_post`, post.`date_published`, post.`title`, post.`under_title`, 
               post.`intro`, post.`contain`, post.`pic`, post.`id_user`, 
               user.`id_user`, user.`username`, user.`profile_picture`
        FROM `post` 
...
<body>
    ...
    <main>
        ...
        <article>
            <h2>
                <a href="./index.php"><img src="<?= htmlspecialchars($article['profile_picture']) ?>" alt="Photo de profil">
                    <?= htmlspecialchars($article['username']) ?></a>
            </h2>
            <p>Publié le : <?= htmlspecialchars($article['date_published']) ?></p>
            <h1><?= htmlspecialchars($article['title']) ?></h1>
            <img src="<?= htmlspecialchars($article['pic']) ?>" alt="Photo de l'article">
            <p><?= htmlspecialchars($article['intro']) ?></p>
            <h3><?= htmlspecialchars($article['under_title']) ?></h3>
            <p><?= htmlspecialchars($article['contain']) ?></p>


            <?php if ($id_user === $article['id_user']): ?> <a href="edit_article.php?id=<?= $article['id_post'] ?>" class="btn btn-primary">Modifier l'article</a> <a href="delete_article.php?id=<?= $article['id_post'] ?>" class="btn btn-danger" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet article ?')">Supprimer l'article</a> <?php endif; ?>
        </article>
    </main>
    <footer>
        <hr>
